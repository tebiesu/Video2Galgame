...
def _resolve_bilibili_media_url(video_url: str) -> str:
    ytdlp_bin = _resolve_executable('yt-dlp', 'YTDLP_BIN')
    cmd = [ytdlp_bin, '-g', '-f', 'bestaudio/best', video_url]
    user_cookie = os.environ.get('BILIBILI_COOKIE', '').strip()
    if user_cookie:
        cmd.extend(['--add-header', f'Cookie: {user_cookie}'])
    proc = subprocess.run(cmd, capture_output=True, text=True, timeout=90, check=True)
    lines = [x.strip() for x in proc.stdout.splitlines() if x.strip()]
    if not lines:
        raise RuntimeError('yt-dlp 未返回媒体流地址。')
    return lines[0]


def extract_audio_for_asr_bilibili(video_url: str, output_path: str, max_duration_sec: int = 3600) -> tuple[bool, str]:
    try:
        media_url = _resolve_bilibili_media_url(video_url)
...
def transcribe_with_siliconflow_bilibili(video_input: str, bvid: str) -> tuple[str, str]:
    ...
    video_url = resolve_short_url(video_input)
    if not extract_bvid(video_url):
        video_url = f'https://www.bilibili.com/video/{bvid}'

    endpoint = f"{_siliconflow_base_url()}/audio/transcriptions"
    model = _siliconflow_asr_model()

    with tempfile.TemporaryDirectory(prefix='bili_asr_') as td:
        audio_path = str(Path(td) / f'{bvid}.mp3')
        ok, reason = extract_audio_for_asr_bilibili(video_url, audio_path, max_duration_sec=3600)
        if not ok:
            return '', reason
...
def process_bilibili_url(video_input: str, lang_code: str | None = None) -> str:
    ...
    Returns plain text subtitle content; falls back to SiliconFlow ASR if enabled.
    """
    user_cookie = os.environ.get('BILIBILI_COOKIE')
    ...
    try:
        ...
        try:
            ...
            if isinstance(subtitle_data, dict) and 'body' in subtitle_data and isinstance(subtitle_data['body'], list):
                ...
                # Subtitle APIs returned empty; fallback to ASR
                asr_text, asr_reason = transcribe_with_siliconflow_bilibili(video_input, bvid)
                if asr_text:
                    return "（未获取到原生字幕，以下为 ASR 转写内容）\n" + asr_text
                logging.warning(f"ASR fallback failed for BVID {bvid}: {asr_reason}")
                return f"（未获取到字幕内容）\n原因: 字幕接口返回空 | {asr_reason}"
            else:
                logging.warning(f"Subtitle JSON for BVID {bvid} has unexpected structure or is missing 'body'. Raw: {subtitle_json_string[:100]}...")
                asr_text, asr_reason = transcribe_with_siliconflow_bilibili(video_input, bvid)
                if asr_text:
                    return "（原生字幕结构异常，以下为 ASR 转写内容）\n" + asr_text
                return f"（未获取到字幕内容）\n原因: 字幕JSON结构异常 | {asr_reason}"
        except json.JSONDecodeError:
            logging.error(f"Failed to decode subtitle JSON for BVID {bvid}. Raw: {subtitle_json_string[:100]}...")
            asr_text, asr_reason = transcribe_with_siliconflow_bilibili(video_input, bvid)
            if asr_text:
                return "（原生字幕解析失败，以下为 ASR 转写内容）\n" + asr_text
            return f"（未获取到字幕内容）\n原因: 字幕JSON解析失败 | {asr_reason}"
        except Exception as parse_e:
             logging.exception(f"Unexpected error processing subtitle JSON for BVID {bvid}: {parse_e}")
             asr_text, asr_reason = transcribe_with_siliconflow_bilibili(video_input, bvid)
             if asr_text:
                 return "（原生字幕处理失败，以下为 ASR 转写内容）\n" + asr_text
             return f"（未获取到字幕内容）\n原因: 字幕处理异常 | {asr_reason}"

    except Exception as e:
        logging.exception(f"Error processing Bilibili URL {video_input}: {e}")
        asr_text, asr_reason = transcribe_with_siliconflow_bilibili(video_input, bvid)
        if asr_text:
            return "（原生字幕流程异常，以下为 ASR 转写内容）\n" + asr_text
        return f"（未获取到字幕内容）\n原因: 原生字幕流程异常 | {asr_reason}"
